...
export class WebUsbLinuxPort implements WebUsbPortInterface {
    device: any;
    decoder: any;
    encoder: any;
    rawMode: boolean;
    ...
    public connect(): Promise<void> {
        ...
        return new Promise<void>((resolve, reject) => {
            let readLoop = () => {
                this.device.transferIn(1, 64).then((result: any) => {
                    let skip = true,
                        skip_prompt = true,
                    ...
                    if (!this.ashellReady)
                        this.ashellReady = /^(\$gshell\>)/.test(str);
                    ...
                    console.log("skip_prompt:", skip_prompt, this.echoMode);
...
//                    this.onReceive(str);
    ...
    public read(): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            this.device.transferIn(1, 64).then((response: any) => {
    ...
    public send(data: string): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            ...
            this.device.transferOut(1, this.encoder.encode(data))
            .then(() => { resolve(); })
